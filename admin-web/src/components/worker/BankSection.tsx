...
export default function BankSection({ values, onChange }: BankSectionProps) {
  return (
      ...
      <div className="flex items-center justify-between">
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            ...
            onChange={(e) => onChange("enable_bank", e.target.checked)}
          />
          Enable
        </label>
      </div>
      ...
      {values.enable_bank && (
        <div className="mt-6 space-y-5">
          ...
          <QRUploader
            ...
            onUploaded={(url) => onChange("bank_qr", url)}
          />
        </div>
      )}
    </div>
