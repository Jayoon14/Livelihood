...
export default function Payment() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const paymentData = await getWorkerPaymentInformation(
  ...
  async function handlePay() {
    ...
    try {
      ...
      const bookingData = booking ?? (await getBookingById(Number(id)));
  ...
  return (
    <CustomerLayout>
      <div className="max-w-xl mx-auto bg-white rounded-2xl shadow p-8">
        ...
        {/* Payment Details */}
        <div className="mt-6">
          {method === "GCash" && paymentInfo.enable_gcash && (
            <div className="rounded-xl border p-5 space-y-4">
              <h2 className="font-bold text-lg">GCash Payment</h2>

              <p>
                <strong>Name:</strong> {paymentInfo.gcash_name}
              </p>

              <p>
                <strong>Number:</strong> {paymentInfo.gcash_number}
              </p>
          ...
          {method === "Maya" && paymentInfo.enable_maya && (
            <div className="rounded-xl border p-5 space-y-4">
              <h2 className="font-bold text-lg">Maya Payment</h2>

              <p>
                <strong>Name:</strong> {paymentInfo.maya_name}
              </p>

              <p>
                <strong>Number:</strong> {paymentInfo.maya_number}
              </p>
          ...
          {method === "Bank Transfer" && paymentInfo.enable_bank && (
            <div className="rounded-xl border p-5 space-y-4">
              <h2 className="font-bold text-lg">Bank Transfer</h2>

              <p>
                <strong>Bank:</strong> {paymentInfo.bank_name}
              </p>

              <p>
                <strong>Account Name:</strong> {paymentInfo.account_name}
              </p>

              <p>
                <strong>Account Number:</strong> {paymentInfo.account_number}
              </p>
        ...
        {method !== "Cash" && (
          <div className="mt-6 space-y-5">
            ...
            <div>
              <label className="font-semibold">Upload Proof of Payment</label>
