      ...
      <div className="p-8 space-y-6">
        ...
        {leftScore > rightScore && leftWorker && (
          <div className="bg-green-100 text-green-700 rounded-xl p-4 text-center font-bold">
            🏆 Recommended Worker: {leftWorker.profile?.first_name}
          </div>
        )}

        {rightScore > leftScore && rightWorker && (
          <div className="bg-green-100 text-green-700 rounded-xl p-4 text-center font-bold">
            🏆 Recommended Worker: {rightWorker.profile?.first_name}
          </div>
        )}

        {leftScore === rightScore && leftWorker && rightWorker && (
        ...
        <div className="grid grid-cols-2 gap-8">
          ...
          <div
            ...
            `}
          >
            <h2 className="text-xl font-semibold mb-4">Worker A</h2>
            ...
            {leftWorker && (
              <div className="space-y-3">
                ...
                {priceWinner === "left" && (
                  <div className="text-green-600 font-bold">🏆 Best Price</div>
                )}

                <p>
                  <b>Category:</b> {leftWorker.services?.[0]?.category ?? "-"}
                </p>

                <p>
                  <b>Experience:</b> {leftWorker.workExperience?.length ?? 0}{" "}
                  jobs
                </p>

                <p>
                  <b>Rating:</b> ⭐ {leftRating}
                </p>

                <p>
                  <b>Price:</b> ₱{leftWorker.services?.[0]?.price ?? "-"}
                </p>
                ...
                <button
                  onClick={() => navigate(`/customer/book/${leftId}`)}
          ...
          <div
            ...
            `}
          >
            <h2 className="text-xl font-semibold mb-4">Worker B</h2>
            ...
            {rightWorker && (
              <div className="space-y-3">
                ...
                <p>
                  <b>Category:</b> {rightWorker.services?.[0]?.category ?? "-"}
                </p>

                <p>
                  <b>Experience:</b> {rightWorker.workExperience?.length ?? 0}{" "}
                  jobs
                </p>

                <p>
                  <b>Rating:</b> ⭐ {rightRating}
                </p>

                <p>
                  <b>Price:</b> ₱{rightWorker.services?.[0]?.price ?? "-"}
                </p>
                ...
                <button
                  onClick={() => navigate(`/customer/book/${rightId}`)}
                  className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg"
                >
                  Book This Worker
                </button>
